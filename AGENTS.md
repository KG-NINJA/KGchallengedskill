...
## AIEO portfolio and pipelines

For AIEO changes read `.github/workflows/aieo_effect_checks.yml`; its regression
command is `python -m pytest tests/test_aieo_visibility_tracker.py tests/test_aieo_effect_compose.py tests/test_aieo_composite_tracker.py tests/test_aieo_memory_engine.py tests/test_aieo_workflow_concurrency.py tests/test_aieo_workflow_guards.py`.
Use the relevant `README.md` topic and affected workflow for context. Preserve
scheduled/generated-data state; do not dispatch analytics or generation merely
to validate instructions. `AGENTS_patch.txt` is a dummy API example.
Use Japanese comments and the existing blue (#007BFF) visual guideline.
Read [the workflow reference](docs/agent-workflow-reference.md) only when an
optional progress record helps a complex implementation; it grants no new scope.
